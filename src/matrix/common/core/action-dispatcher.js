...
import visualApi from "../../common/api/visualApi.js";
...
export class ActionDispatcher {
    ...
    constructor(resolver, eventBus) {
        ...
        // 如需新增动作处理逻辑, 请在此添加
        this.handlers = {
            [ActionType.OpenUrl]: (ele, action) =>
                this.openUrl(ele, action?.actionOptions?.url),
            [ActionType.ChangeColor]: (ele, action) =>
                this.changeColor(ele, action?.actionOptions?.color),
            [ActionType.PopComponent]: (ele, action, position) =>
                this.popComponent(
                    ele,
                    action?.actionOptions?.name,
                    action?.actionOptions?.props,
                    position
                ),
            [ActionType.ChangeScene]: (ele, action) =>
                this.changeScene(action?.actionOptions?.sceneId),
            [ActionType.ExecuteScript]: (ele, action) =>
                this.executeScript(ele, action?.actionOptions?.url),
            [ActionType.ChangeAnchor]: (ele, action) =>
                this.changeAnchor(action?.actionOptions?.anchorId),
            [ActionType.ChangePosition]: (ele, action) =>
                this.changePosition(
                    ele,
                    action?.actionOptions?.position,
                    action?.actionOptions?.duration
                ),
            [ActionType.ChangeVisible]: (ele, action) =>
                this.changeVisible(ele, action?.actionOptions?.visible),
            [ActionType.RunEleBehavior]: (ele, action) =>
                this.runEleBehavior(
                    ele,
                    action?.actionOptions?.behaviorName,
                    action?.actionOptions?.behaviorParam
                ),
        };
    }
    ...
    dispatch(ele, action, position) {
        ...
        let actionType = ActionTypeMeta[action.actionType];
        if (!actionType) {
            console.warn(
        ...
        if (!checkProps(action.actionOptions, actionType.scheme)) {
            console.warn(
                `动作${action.actionType}必须传递${actionType.scheme.join(
                    ","
                )}属性, 请在actionOptions传递指定参数`
            );
            return;
        }
    ...
    popComponent(ele, compName = "", props = {}, position) {
        ...
        if (!compName) {
            console.warn(
                `id:${ele.id}, actionType: popComponent, actionOptions缺少'name'组件名`
            );
            return;
        }

        // 组件未定义
        if (!PopComponent[compName]) {
            console.warn(
                `组件${compName}未定义, 请在const.js的PopComponent中定义组件`
            );
            return;
        }

        let component = PopComponentMeta[compName];

        // 校验组件所需属性
        if (!checkProps(props, component.props)) {
            console.warn(
                `组件${component.name}必须传递${component.props.join(
                    ","
                )}属性, 请在actionOptions.props传递指定参数`
            );
            return;
        }
