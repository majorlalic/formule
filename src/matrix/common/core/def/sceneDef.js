import { ElementDef } from "./elementDef.js";
...
export class SceneDef {
    ...
    runEleBehavior(eleId, behaviorName, behaviorParam) {
        ...
        // 2. 校验图元动作定义
        if (!ElementBehaviorMeta.hasOwnProperty(behaviorName)) {
            console.log(
                `图元动作尚未定义${behaviorName}, 请在ElementBehavior中定义`
            );
            return;
        }
        ...
        if (
            ...
            console.log(`当前图元${ele.id}不存在${behaviorName}, 请联系管理员`);
            return;
        }

        let behavior = ElementBehaviorMeta[behaviorName];
        ...
        // TODO 如何保证参数的顺序
        // 6. 执行动作
        ele[behaviorName](...Object.values(paramObj));
    }
